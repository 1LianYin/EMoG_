...
class GaussianDiffusion:
    ...
    def p_sample(
        ...
        if pre_seq is not None:
            T = pre_seq.shape[1]
            noise = th.randn_like(pre_seq.float())
    ...
    def p_sample_loop(
        ...
        for sample in self.p_sample_loop_progressive(
            ...
            final = sample
        return final["sample"] # pred_xstart
                ...
                if resizers is not None:
                    if i > range_t:
                        out["sample"] = out["sample"] - up(down(out["sample"])) + up(
                            down(self.q_sample(model_kwargs["ref_seq"], t, th.randn(*shape, device=device))))
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            try:
                model_output, pred_logits = model(x_t, self._scale_timesteps(t), **model_kwargs)
                terms['pred_logits'] = pred_logits
            except:
                model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
                terms['pred_logits'] = None
