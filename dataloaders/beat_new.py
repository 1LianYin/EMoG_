...
class CustomDataset(Dataset):
    def __init__(self, args, loader_type, augmentation=None, kwargs=None, build_cache=True):
        ...
        self.pose_cache = args.pose_cache
        ...
        if loader_type == "train":
            ...
            preloaded_dir = self.data_dir + f"{self.pose_cache}"
            logger.info(f"Using cache: {self.pose_cache}")
        elif loader_type == "val":
            self.data_dir = args.test_data_path
            self.multi_length_training = [1.0] 
            preloaded_dir = self.data_dir + "bvh_rot_cache"
            logger.info(f"Using cache: bvh_rot_cache")
        else:
            self.data_dir = args.test_data_path
            self.multi_length_training = [1.0]
            preloaded_dir = self.data_dir + "bvh_rot_cache"
        ...
        if self.word_rep is not None:
            with open("/code/MotionDiffuse/text2motion/dataloaders/vocab.pkl", 'rb') as f:
        ...
        self.audio_norm = False #args.audio_norm
        self.facial_norm = False #args.facial_norm
        if self.audio_norm:
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        for pose_file in pose_files:
            ...
            if self.word_rep is not None:
                ...
                caption = ''
                for i in range(pose_each_file.shape[0]): # 825
                    found_flag = False
            ...
            filtered_result = self._sample_from_clip(
                dst_lmdb_env,
                audio_each_file, pose_each_file, facial_each_file, word_each_file, text_each_file,
    ...
    def _sample_from_clip(
        self, dst_lmdb_env, audio_each_file, pose_each_file, facial_each_file, word_each_file, text_each_file,
        vid_each_file, emo_each_file, sem_each_file,
        disable_filtering, clean_first_seconds, clean_final_seconds, is_test,
        ):
        """
        ...
        for ratio in self.multi_length_training:
            logger.info(f"ratio {ratio}, length {self.pose_length * ratio}")
            ...
            for i in range(num_subdivision): # cut into around 2s chip, (self npose)
                ...
                fin_idx = start_idx + self.pose_length
                ...
                sample_caption = ''
                if text_each_file != []:
                    for index in range(start_idx + 1, fin_idx):
                        if index <= start_idx and index > fin_idx:
                            continue
                           
                        elif text_each_file[index - 1] != text_each_file[index]:
                            if text_each_file[index - 1] == ' ':
                                sample_caption += text_each_file[index - 1]
                            else: sample_caption = sample_caption + text_each_file[index - 1] + ' '
                            
                            if index == fin_idx - 1:
                                sample_caption += text_each_file[index]
                ...
                if sample_pose.any() != None:
                    ...
                    if is_correct_motion or disable_filtering:
                        ...
                        sample_text_list.append(sample_caption)
            ...
            if len(sample_pose_list) > 0:
                with dst_lmdb_env.begin(write=True) as txn:
                    for pose, audio, facial, word, text, vid, emo, sem in zip(sample_pose_list,
                                                        sample_audio_list,
                                                        sample_facial_list,
                                                        sample_word_list,
                                                        sample_text_list,
                        ...
                        v = [normalized_pose, audio, facial, word, text, emo, sem, vid]
    ...
    def __getitem__(self, idx):
        with self.lmdb_env.begin(write=False) as txn:
            ...
            try:
                tar_pose, in_audio, in_facial, in_word, text, emo, sem, vid = sample
            except:
            ...
            if self.loader_type == "test":
                ...
                m_length = tar_pose.shape[0]
                max_motion_length = self.args.pose_length 
                if m_length >= self.args.pose_length:
                    idx = random.randint(0, len(tar_pose) - max_motion_length)
                    tar_pose = tar_pose[idx: idx + max_motion_length]
                else:
                    """
                    for audio sr = 16000, fps = 15, pose_length = 34, 
                    audio short length = 36266.7 -> 36266
                    ...
                    """
                    D = tar_pose.shape[1]
                    padding_len = int(max_motion_length - m_length)
                    aud_padding_len = int((padding_len / self.args.pose_fps) * self.args.audio_fps)

                    padding_zeros = torch.zeros(padding_len, D)
                    aud_padding_zeros = torch.zeros(aud_padding_len)
                    facial_padding_zeros = torch.zeros(padding_len, in_facial.shape[1])
                    sem_padding_zeros = torch.zeros(padding_len)
                    emo_padding = torch.full((padding_len, ), emo[0])

                    tar_pose = torch.cat((tar_pose, padding_zeros), dim=0)
                    in_audio = torch.cat((in_audio, aud_padding_zeros), dim=0)
                    in_facial = torch.cat((in_facial, facial_padding_zeros), dim=0)
                    in_word = torch.cat((in_word, sem_padding_zeros), dim=0).int()
                    emo = torch.cat((emo, emo_padding), dim=0).int()
                    sem = torch.cat((sem, sem_padding_zeros), dim=0)

            return {"pose":tar_pose, "length":m_length, "audio":in_audio, "facial":in_facial, "word":in_word, "id":vid, "emo":emo, "sem":sem}
...
class MotionPreprocessor:
    ...
    def check_pose_diff(self, verbose=False):
        print(self.skeletons.shape, self.mean_pose.shape)
