...
def calculate_diversity(activation, diversity_times):
    ...
    dist = linalg.norm(activation[:diversity_times] - activation[first_indices], axis=0)
    return dist.mean()
...
class mDiversity(object):
    def __init__(self):
        self.counter = 0
        self.sum = 0
        
    def run(self, motion_embeddings, diversity_times):
        div = calculate_multimodality(motion_embeddings, diversity_times)
        self.counter += motion_embeddings.shape[0]
        self.sum += div
        return div
...
class Diversity(object):
    ...
    def run(self, motion_embeddings, diversity_times):
        div = calculate_diversity(motion_embeddings, diversity_times)
        self.counter += motion_embeddings.shape[0]
        self.sum += div
        return div
